...
<title>tryaii-dre eval &mdash; {summary['totalPrompts']} prompts</title>
<style>
{_DASHBOARD_STYLE}
</style>
</head>
...
def cmd_eval(args):
    ...
    from tryaii_dre import Priorities, Router
    from tryaii_dre.budget import route_dataset_with_budget

    if not getattr(args, "verbose", False):
        logging.getLogger("tryaii_dre").setLevel(logging.WARNING)
    ...
    if args.output:
        ...
        stamp = time.strftime("tryaii-dre-eval-%Y%m%d-%H%M%S")
